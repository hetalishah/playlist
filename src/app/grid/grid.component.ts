...
export class GridComponent {
  ...
  exportChart(): void {
    this.chart.exportImage({
      ...
      saveAs(dataURI, 'genre-pie-chart.png');
    });
  }
  ...
  public open({ dataItem, columnIndex, editedRowIndex, rowIndex }) {
    if (columnIndex !== 4 && !this.editedRowIndex && rowIndex !== -1) {
      ...
      utube.subscribe((response) => {
        ...
        let iframe = document.getElementById('myIframe').setAttribute('src', 'https://www.youtube.com/embed/' + ids + '?autoplay=1&enablejsapi=1');
      })
    }
  }
}